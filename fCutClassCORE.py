	...
	def cutter(self, conv = False, step = 25):
		...
		for i in irange[:len(irange) - (self.xwindow / step - 1) * conv]:
			for j in jrange[:len(jrange) - (self.ywindow / step - 1) * conv]:
				window = self.imgArray[i * (self.xwindow * (not(conv)) + step * conv) : i * (self.xwindow * (not(conv)) + step * conv) + self.xwindow, j * (self.ywindow * (not(conv)) + step * conv) : j * (self.ywindow * (not(conv)) + step * conv) + self.ywindow]
